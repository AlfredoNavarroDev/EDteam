...
public class TiendaOnline
{
    ...
    public async Task<bool> ConsultarStockAsync(string producto)
    {
        ...
        int disponible = _inventario[producto];
    ...
    {
        Console.WriteLine($"Admitiendo tarje: {tarjeta}");
    ...
    {
        Console.WriteLine($"Procesando envio a {direccion}...");
        
        await Task.Delay(1500);

        var tracking = $"TRACK-{Guid.NewGuid().ToString()[..8].ToUpper()}";
        Console.WriteLine($"Pedido enviado: {tracking}");
        return tracking;
    }
    ...
    {
        Console.WriteLine($"Consultando {productos.Count} productos en paralelo...");
    ...
    private async Task<string> SimularProveedorAsync(string nombre, int delay)
    {
        ...
        return $"{nombre} respondió en {delay/1000} segundos";
    }
}